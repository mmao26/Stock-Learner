...
def compute_portvals(orders_file = "./orders-rule.csv", start_val = 100000):
...
def test_code():
    ...
    normed_rule = portvals/portvals.ix[0]

    df_temp = pd.concat([normed_rule, normed_bench], keys=['Rule-Based Portfolio', 'Benchmark'], axis=1)
    ax = df_temp.plot(color=['b','k'])
    ax.set_title('Rule-Based Portfolio and Benchmark')
    ax.set_ylabel('Normalized Price')
    ax.set_xlabel('Date')
    ax.legend(loc=0)
    plt.grid(True)
    plt.axvline(dt.datetime(2008,6,12), color='g', linestyle='dashed', linewidth=1.5)
    plt.axvline(dt.datetime(2008,7,18), color='g', linestyle='dashed', linewidth=1.5)
    plt.axvline(dt.datetime(2008,9,4), color='g', linestyle='dashed', linewidth=1.5)
    plt.axvline(dt.datetime(2008,12,22), color='g', linestyle='dashed', linewidth=1.5)
    plt.show()
